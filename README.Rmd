...
Likewise, in data visualization products, we see how repetitive recoding can be in the Titanic data example that follows. 
...
Given how verbose recoding an indicator variable can be, analysts may choose to forego a recoding the variable, especially in exploratory analysis.
...
Below, the table that is created by using the indicator variable directly is awkward to interpret.  The indicator variable name persists in the output allowing savvy readers to interpret the output, but communication is strained. 
...
```{r manipulation_ind2cat_cat_true}
